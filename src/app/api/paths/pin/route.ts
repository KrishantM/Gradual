/**
 * POST /api/paths/pin
 * Body: { pathId: string, pinned: boolean }
 *
 * Toggle the pinned flag on a user's path_state. Pinned paths win the
 * "active path" slot on the dashboard regardless of recency.
 */

import { NextRequest, NextResponse } from 'next/server';
import { z } from 'zod';
import { auth, db } from '@/lib/firebase-admin';

const BodySchema = z.object({
  pathId: z.string().min(1).max(120),
  pinned: z.boolean(),
});

export async function POST(req: NextRequest) {
  try {
    const authHeader = req.headers.get('authorization');
    if (!authHeader?.startsWith('Bearer ')) {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }
    const decoded = await auth.verifyIdToken(authHeader.slice(7));
    const uid = decoded.uid;

    const raw = await req.json();
    const parseResult = BodySchema.safeParse(raw);
    if (!parseResult.success) {
      return NextResponse.json({ error: 'Invalid body', details: parseResult.error.flatten() }, { status: 400 });
    }
    const { pathId, pinned } = parseResult.data;

    const ref = db.collection('users').doc(uid).collection('path_state').doc(pathId);
    const snap = await ref.get();
    if (!snap.exists) {
      return NextResponse.json({ error: 'Not enrolled' }, { status: 404 });
    }

    await ref.update({ pinned, lastActivityAt: new Date() });
    return NextResponse.json({ ok: true, pinned });
  } catch (e) {
    console.error('[POST /api/paths/pin]', e);
    return NextResponse.json({ error: 'Failed to update pin' }, { status: 500 });
  }
}
